...
using Aas.TwinEngine.Plugin.RelationalDatabase.Infrastructure.Providers.ManifestProvider.Helper;
...
public class ManifestProvider(ILogger<ManifestProvider> logger) : IManifestProvider
{
    ...
    {
        if (_mappingJson is null)
        {
            logger.LogWarning("Mapping JSON is null - returning empty semantic id list.");
            return [];
        }
